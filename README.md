...
Upgrade later with `omni-kubeconfig update` or re-run the install script. On interactive runs, the tool checks for a newer stable release and prompts to install (disable with `--no-update-check` or `OMNI_KUBECONFIG_SKIP_UPDATE_CHECK=1`). Confirming the prompt restarts into the new binary and continues your command (Unix: `exec`; Windows: spawn + exit). `update` skips downloading when already on the latest release.
...
| Tag | When |
|-----|------|
| `v0.3.6-snapshot` | Floating latest from `main` for the current release line (after Trivy) |
| `v0.3.6-snapshot.N` | Immutable numbered snapshot for a specific `main` build (after Trivy) |
...
```bash
# After merging to main (release line v0.3.6)
docker pull ghcr.io/jubblin/omni-kubeconfig:v0.3.6-snapshot
docker pull ghcr.io/jubblin/omni-kubeconfig:v0.3.6-snapshot.3
...
| Flag | Default | Description |
|------|---------|-------------|
| `--version` | latest stable (if newer) | Install a specific release tag (e.g. `v0.3.6`); latest path skips when already current |
