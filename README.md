...
| Tag | When |
|-----|------|
| `v0.3.5-snapshot` | Floating latest from `main` for the current release line (after Trivy) |
| `v0.3.5-snapshot.N` | Immutable numbered snapshot for a specific `main` build (after Trivy) |
...
```bash
# After merging to main (release line v0.3.5)
docker pull ghcr.io/jubblin/omni-kubeconfig:v0.3.5-snapshot
docker pull ghcr.io/jubblin/omni-kubeconfig:v0.3.5-snapshot.3
...
| Flag | Default | Description |
|------|---------|-------------|
| `--version` | latest stable (if newer) | Install a specific release tag (e.g. `v0.3.5`); latest path skips when already current |
